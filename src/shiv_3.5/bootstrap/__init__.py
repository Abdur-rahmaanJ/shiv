...
def cache_path(archive, root_dir, build_id):
    ...
    root = root_dir or os.path.expanduser("~/.shiv")
    # name = Path(archive.filename).resolve().name
    # name = resolve(archive.filename).name
    name = os.path.abspath(archive.filename)
    name = os.path.basename(os.path.normpath(name))
    # return root / "{name}_{build_id}".format(name=name, build_id=build_id)
    return os.path.join(root, "{name}_{build_id}".format(name=name, build_id=build_id))
...
def extract_site_packages(archive, target_path, compile_pyc=False, compile_workers=0, force=False):
    ...
    # lock = Path(parent, ".{targetp}_lock".format(targetp=target_path.name))

    # target_path_tmp = os.path.join(parent, target_path.name + ".tmp")

    target_path = os.path.basename(os.path.normpath(target_path))
    target_path_tmp = os.path.join(parent, target_path + ".tmp")
    lock = os.path.join(parent, ".{targetp}_lock".format(targetp=target_path))
...
def bootstrap():  # pragma: no cover
    ...
    with current_zipfile() as archive:
        ...
        site_packages = os.path.join(cache_path(archive, env.root, env.build_id), "site-packages")

        # determine if first run or forcing extract
        if not os.path.exists(site_packages) or env.force_extract:
