...
def execute_content(name, content, argv0=None):
    ...
    try:
        ...
        raise RuntimeError("Unable to parse {name}. Is it a Python script? Syntax correct?".format(name=name))
        ...
        if arg == "-c":
            ...
            if arg == "-":
                ...
                try:
                    ...
                    raise RuntimeError("Could not open '{arg}' in the environment [{sysarg}]: {e}".format(arg=arg, e=e, sysarg=sys.argv[0]))
